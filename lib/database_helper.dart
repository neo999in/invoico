...
class DatabaseHelper {
  ...
  Future _createDB(Database db, int version) async {
    ...
    // Inject 2 Weeks of realistic business data
    await _seedData(db);
  }

  Future<void> _seedData(Database db) async {
    // ==========================================
    // 1. SETUP: PROFILE, SUPPLIERS, CUSTOMERS
    // ==========================================
    await db.insert('suppliers', {'name': 'Ramesh Distributors', 'phone': '9876543210', 'address': 'Andheri East', 'balance_due': 0.0});
    await db.insert('suppliers', {'name': 'Adani Wilmar Dist', 'phone': '9876543211', 'address': 'Vashi Market', 'balance_due': 0.0});
    await db.insert('customers', {'name': 'Walk-in Customer', 'phone': 'N/A', 'address': 'Local Area'});

    // ==========================================
    // 2. CATEGORIES & PRODUCTS
    // ==========================================
    int idDairy = await db.insert('categories', {'name': 'Dairy', 'gst_rate': 5.0});
    await db.insert('products', {'name': 'Amul Milk', 'price': 32.0, 'purchase_price': 28.0, 'stock': 2000, 'gst_rate': 5.0, 'category_id': idDairy}); // 1
    await db.insert('products', {'name': 'Butter', 'price': 58.0, 'purchase_price': 48.0, 'stock': 1000, 'gst_rate': 5.0, 'category_id': idDairy}); // 2

    int idWheat = await db.insert('categories', {'name': 'Wheats & Rice', 'gst_rate': 5.0});
    await db.insert('products', {'name': 'Basmati Rice', 'price': 120.0, 'purchase_price': 95.0, 'stock': 3000, 'gst_rate': 5.0, 'category_id': idWheat}); // 3
    await db.insert('products', {'name': 'Kolam Rice', 'price': 70.0, 'purchase_price': 55.0, 'stock': 3000, 'gst_rate': 5.0, 'category_id': idWheat}); // 4

    int idDal = await db.insert('categories', {'name': 'Dals & Pulses', 'gst_rate': 5.0});
    await db.insert('products', {'name': 'Moong Dal', 'price': 110.0, 'purchase_price': 85.0, 'stock': 2000, 'gst_rate': 5.0, 'category_id': idDal}); // 5
    await db.insert('products', {'name': 'Toor Dal', 'price': 145.0, 'purchase_price': 115.0, 'stock': 2000, 'gst_rate': 5.0, 'category_id': idDal}); // 6

    int idPack = await db.insert('categories', {'name': 'Packaged Food', 'gst_rate': 18.0});
    await db.insert('products', {'name': 'Wafers', 'price': 20.0, 'purchase_price': 15.0, 'stock': 5000, 'gst_rate': 18.0, 'category_id': idPack}); // 7
    await db.insert('products', {'name': 'Biscuit', 'price': 10.0, 'purchase_price': 7.0, 'stock': 5000, 'gst_rate': 18.0, 'category_id': idPack}); // 8

    int idVeg = await db.insert('categories', {'name': 'Vegetables & Fruits', 'gst_rate': 0.0});
    await db.insert('products', {'name': 'Potato', 'price': 30.0, 'purchase_price': 20.0, 'stock': 3000, 'gst_rate': 0.0, 'category_id': idVeg}); // 9
    await db.insert('products', {'name': 'Onion', 'price': 40.0, 'purchase_price': 25.0, 'stock': 3000, 'gst_rate': 0.0, 'category_id': idVeg}); // 10

    int idHouse = await db.insert('categories', {'name': 'Household', 'gst_rate': 18.0});
    await db.insert('products', {'name': 'Harpic', 'price': 98.0, 'purchase_price': 80.0, 'stock': 1000, 'gst_rate': 18.0, 'category_id': idHouse}); // 11

    int idOil = await db.insert('categories', {'name': 'Edible Oil', 'gst_rate': 5.0});
    await db.insert('products', {'name': 'Fortune Sunflower', 'price': 165.0, 'purchase_price': 145.0, 'stock': 2000, 'gst_rate': 5.0, 'category_id': idOil}); // 12

    // ==========================================================
    // 3. GENERATE 3 MONTHS OF DATA (Dec 25, Jan 26, Feb 26)
    // ==========================================================

    // Using a batch so inserting 500+ invoices doesn't freeze the app
    ...
    for (var m in months) {
      String y = m['year'].toString();
      String mo = m['month'].toString().padLeft(2, '0');

      // ----------------------------------------------------
      // MONTHLY EXPENSES (Total: ₹3,500)
      // ----------------------------------------------------
      batch.insert('expenses', {'title': 'Shop Rent', 'amount': 2500, 'date': '$y-$mo-02 10:00:00'});
      batch.insert('expenses', {'title': 'Electricity Bill', 'amount': 1000, 'date': '$y-$mo-10 09:00:00'});

      // ----------------------------------------------------
      // MONTHLY OTHER INCOME (Total: ₹3,000 - Strictly < 5k)
      // ----------------------------------------------------
      batch.insert('other_incomes', {'title': 'Empty Sacks & Tins Scrap Sale', 'amount': 1500, 'date': '$y-$mo-15 14:00:00'});
      batch.insert('other_incomes', {'title': 'Local Brand Poster Display', 'amount': 1500, 'date': '$y-$mo-20 10:00:00'});

      // ----------------------------------------------------
      // MONTHLY RESTOCK PURCHASES
      // ----------------------------------------------------
      batch.insert('purchases', {'product_name': 'Monthly Bulk Restock', 'supplier': 'Ramesh Distributors', 'quantity': 1000, 'cost': 75000, 'date': '$y-$mo-03 08:30:00'});
      batch.insert('payables', {'title': 'Restock Bill', 'amount': 75000, 'balance_due': 0, 'date': '$y-$mo-03 08:30:00'});
      batch.insert('settlements', {'txn_id': m['month'], 'txn_type': 'payable', 'amount': 75000, 'date': '$y-$mo-04 09:30:00'});

      // ----------------------------------------------------
      // DAILY WALK-IN RETAIL SALES (6 Orders per day)
      // ----------------------------------------------------
      for (int d = 1; d <= m['days']; d++) {
        String day = d.toString().padLeft(2, '0');
        String baseDate = '$y-$mo-$day';

        // Order 1: Basmati & Oil (₹670)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 09:30:00', 'total_amount': 670.0, 'discount': 0, 'paid_amount': 670.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 3, 'product_name': 'Basmati Rice', 'quantity': 3, 'price': 120.0, 'purchase_price': 95.0, 'gst_rate': 5.0, 'line_total': 360.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 6, 'product_name': 'Toor Dal', 'quantity': 1, 'price': 145.0, 'purchase_price': 115.0, 'gst_rate': 5.0, 'line_total': 145.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 12, 'product_name': 'Fortune Sunflower', 'quantity': 1, 'price': 165.0, 'purchase_price': 145.0, 'gst_rate': 5.0, 'line_total': 165.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 670.0, 'date': '$baseDate 09:30:00'});
        invId++;

        // Order 2: Kolam Rice & Dairy (₹572)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 11:15:00', 'total_amount': 572.0, 'discount': 0, 'paid_amount': 572.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 4, 'product_name': 'Kolam Rice', 'quantity': 5, 'price': 70.0, 'purchase_price': 55.0, 'gst_rate': 5.0, 'line_total': 350.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 7, 'product_name': 'Wafers', 'quantity': 5, 'price': 20.0, 'purchase_price': 15.0, 'gst_rate': 18.0, 'line_total': 100.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 1, 'product_name': 'Amul Milk', 'quantity': 2, 'price': 32.0, 'purchase_price': 28.0, 'gst_rate': 5.0, 'line_total': 64.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 2, 'product_name': 'Butter', 'quantity': 1, 'price': 58.0, 'purchase_price': 48.0, 'gst_rate': 5.0, 'line_total': 58.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 572.0, 'date': '$baseDate 11:15:00'});
        invId++;

        // Order 3: Veggies & Cleaning (₹448)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 14:00:00', 'total_amount': 448.0, 'discount': 0, 'paid_amount': 448.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 9, 'product_name': 'Potato', 'quantity': 5, 'price': 30.0, 'purchase_price': 20.0, 'gst_rate': 0.0, 'line_total': 150.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 10, 'product_name': 'Onion', 'quantity': 5, 'price': 40.0, 'purchase_price': 25.0, 'gst_rate': 0.0, 'line_total': 200.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 11, 'product_name': 'Harpic', 'quantity': 1, 'price': 98.0, 'purchase_price': 80.0, 'gst_rate': 18.0, 'line_total': 98.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 448.0, 'date': '$baseDate 14:00:00'});
        invId++;

        // Order 4: Repeat Order 1 for volume (₹670)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 16:30:00', 'total_amount': 670.0, 'discount': 0, 'paid_amount': 670.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 3, 'product_name': 'Basmati Rice', 'quantity': 3, 'price': 120.0, 'purchase_price': 95.0, 'gst_rate': 5.0, 'line_total': 360.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 6, 'product_name': 'Toor Dal', 'quantity': 1, 'price': 145.0, 'purchase_price': 115.0, 'gst_rate': 5.0, 'line_total': 145.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 12, 'product_name': 'Fortune Sunflower', 'quantity': 1, 'price': 165.0, 'purchase_price': 145.0, 'gst_rate': 5.0, 'line_total': 165.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 670.0, 'date': '$baseDate 16:30:00'});
        invId++;

        // Order 5: Repeat Order 2 for volume (₹572)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 18:00:00', 'total_amount': 572.0, 'discount': 0, 'paid_amount': 572.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 4, 'product_name': 'Kolam Rice', 'quantity': 5, 'price': 70.0, 'purchase_price': 55.0, 'gst_rate': 5.0, 'line_total': 350.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 7, 'product_name': 'Wafers', 'quantity': 5, 'price': 20.0, 'purchase_price': 15.0, 'gst_rate': 18.0, 'line_total': 100.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 1, 'product_name': 'Amul Milk', 'quantity': 2, 'price': 32.0, 'purchase_price': 28.0, 'gst_rate': 5.0, 'line_total': 64.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 2, 'product_name': 'Butter', 'quantity': 1, 'price': 58.0, 'purchase_price': 48.0, 'gst_rate': 5.0, 'line_total': 58.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 572.0, 'date': '$baseDate 18:00:00'});
        invId++;

        // Order 6: Evening Snacks & Dal (₹540)
        batch.insert('invoices', {'customer_name': 'Walk-in Customer', 'customer_phone': 'N/A', 'date': '$baseDate 20:15:00', 'total_amount': 540.0, 'discount': 0, 'paid_amount': 540.0, 'balance_due': 0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 3, 'product_name': 'Basmati Rice', 'quantity': 2, 'price': 120.0, 'purchase_price': 95.0, 'gst_rate': 5.0, 'line_total': 240.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 5, 'product_name': 'Moong Dal', 'quantity': 2, 'price': 110.0, 'purchase_price': 85.0, 'gst_rate': 5.0, 'line_total': 220.0});
        batch.insert('invoice_items', {'invoice_id': invId, 'product_id': 7, 'product_name': 'Wafers', 'quantity': 4, 'price': 20.0, 'purchase_price': 15.0, 'gst_rate': 18.0, 'line_total': 80.0});
        batch.insert('settlements', {'txn_id': invId, 'txn_type': 'receivable', 'amount': 540.0, 'date': '$baseDate 20:15:00'});
        invId++;
      }
    }

    // Commit all 2500+ database rows instantly without freezing the app
    await batch.commit(noResult: true);
    print('Database successfully seeded! AOV < 1000, Other Income < 5000, Net Profit > 20000 achieved!');
  }
  // ====================================================
  //               INVOICE METHODS
  // ====================================================
